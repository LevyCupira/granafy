...
function renderDividas() {
  ...
  var divs = c.dividas || [];
  var tTotal = divs.reduce((s, d) => s + Number(d.total || 0), 0);
  ...
  divs.forEach((_, i) => {
    var el = document.getElementById('dv-pag-data-' + i);
    if (el) el.value = hoje;
  });
...
function calcularBC() {
  ...
  var rows = r.tabela.map(row =>
    '<tr><td>' + row.n + '</td><td>' + fmt(row.pmt) + '</td><td style="color:var(--warning)">' + fmt(row.juros) + '</td><td style="color:var(--accent)">' + fmt(row.amort) + '</td><td>' + fmt(row.saldo) + '</td></tr>'
  ).join('');
...
function usarCalcNaDivida() {
  ...
  var taxaEl = document.getElementById('dv-taxa');
  var vpEl = document.getElementById('dv-vparcela');

  if (pvEl) {
    var c1 = Math.round(_lastCalc.totalPago * 100);
    pvEl.dataset.cents = String(c1);
    pvEl.value = _lastCalc.totalPago.toLocaleString('pt-BR', { minimumFractionDigits: 2 });
  }
  if (nEl) nEl.value = _lastCalc.n;
  if (taxaEl) taxaEl.value = _lastCalc.taxa;
  if (vpEl) {
    var c2 = Math.round(_lastCalc.pmt * 100);
    vpEl.dataset.cents = String(c2);
    vpEl.value = _lastCalc.pmt.toLocaleString('pt-BR', { minimumFractionDigits: 2 });
  }
}
...
function autoParcela() {
  ...
  var vpEl = document.getElementById('dv-vparcela');
  if (vpEl) {
    var cents = Math.round(r.pmt * 100);
    vpEl.dataset.cents = String(cents);
    vpEl.value = r.pmt.toLocaleString('pt-BR', { minimumFractionDigits: 2 });
  }
}
...
async function addDivida() {
  ...
  if (!org || !total) {
    return alert('Preencha pelo menos Órgão/Credor e Valor total.');
  }
  ...
  const payload = {
    ...
    parcelas_total: Number(parcelas || 0),
    parcelas_restantes: Number(parcelas || 0),
  ...
  if (error) {
    ...
    alert('Não foi possível cadastrar a dívida.');
    return;
  }
...
async function deleteDivida(i) {
  ...
  const divida = c.dividas[i];
  if (!divida || !divida.id) return;
...
async function registrarPagamentoDivida(i) {
  ...
  if (!valor || valor <= 0) {
    return alert('Informe o valor pago.');
  }
  ...
  if (!d || !d.id) {
    return alert('Dívida não encontrada.');
  }

  const novoValorPago = Number(d.pago || 0) + Number(valor || 0);
  ...
  if (erroUpdate) {
    console.error('Erro ao atualizar pagamento da dívida:', erroUpdate);
    alert('Não foi possível registrar o pagamento da dívida.');
    return;
  }
  ...
  if (conta !== 'nenhuma') {
    const payloadLancamento = {
      cliente_id: activeClient,
      data_lancamento: data_,
      descricao: 'Parcela — ' + d.org + (d.tipo ? ' (' + d.tipo + ')' : ''),
      categoria: 'Empréstimo / Financiamento',
      tipo: 'debito',
      valor: Number(valor || 0)
    };

    const { error: erroLanc } = await supabaseClient
      .from('lancamentos')
      .insert([payloadLancamento]);

    if (erroLanc) {
      console.error('Erro ao lançar pagamento no extrato:', erroLanc);
      alert('Pagamento da dívida salvo, mas o lançamento no extrato falhou.');
    }
  }
...
function toggleDvHist(i) {
  var el = document.getElementById('dv-hist-' + i);
  if (!el) return;
