...
function renderExtrato() {
  ...
  var html = billsHtml
    ...
    + '<div class="form-group" style="max-width:160px"><label>Data fim</label><input type="date" id="filtro-fim"/></div>'
    + '</div>'
    + '<button class="btn-add" onclick="aplicarFiltroConta()">Filtrar</button>'
    + '<button class="btn-sm" style="margin-left:10px" onclick="limparFiltroConta()">Limpar</button>'
    + '</div>'
  ...
  if (lncs.length === 0) {
    ...
    html += buildTable(
      'extrato',
      cols,
      sorted.map(l => Object.assign({}, l, { _saldo: sm[l._i] })),
  ...
  var di = document.getElementById('ex-data');
